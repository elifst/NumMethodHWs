clc
clear all
w=1.25;                          % relaxiation factor for SOR %
h=pi./10;
k=pi./20;
gama=(h./k).^2;
...
for k=1:5000
    ...
    u(1,:,k+1)=u_xpi2;
    u(11,:,k+1)=u_x0;
    u(i,j,k+1)=(1-w).*u(i,j,k)+(w./(2+2.*gama)).*(-1.*h.*h.*f(i,j)+u(i,j+1,k)+u(i,j-1,k+1)+gama.*(u(i+1,j,k)+u(i-1,j,k+1)));
...
%%%%%%%%%%%%%%%%%%%%%%%%5
fig=figure('Name','Gauss Seidel + SOR');
[x,y] = meshgrid(x,y);
surf(x,y,u(:,:,k+1))
title('Gauss Seidel + SOR for w=1.25','FontSize',20)
...
print('Gauss Seidel and SOR', '-dpdf','-fillpage');
