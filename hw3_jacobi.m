...
for k=1:1000
    for j=2:10
        for i=2:10
    u(i,j,k+1)=(1/(2+2.*gama)).*(-1.*h.*h.*f(i,j)+u(i,j+1,k)+u(i,j-1,k)+gama.*(u(i+1,j,k)+u(i-1,j,k)));
    u(1:11,1,k+1)=u_0y;
    u(1:11,11,k+1)=u_piy;
    u(1,:,k+1)=u_xpi2;
    u(11,:,k+1)=u_x0;
...
%%%%%%%%%%%%%%%%%%%%%%%%5
fig=figure('Name','Jacobi Iteration');
[x,y] = meshgrid(x,y);
surf(x,y,u(:,:,k+1))
title('Jacobi Iteration','FontSize',20)
...
print('Jacobi Iteration', '-dpdf','-fillpage');
